...
public class SignUpController {
    ...
    private void handleSignup() {
        ...
        if (!password.equals(confirmPassword)) {
            displayError("Passwords do not match."); // Show error if passwords don't match
            return;
        }

        if (isUsernameTaken(username)) {
            displayError("Username already taken. Please choose another one."); // Show error if the username is taken
            return;
        }

        List<String> selectedCategories = getSelectedCategories(); // Get selected categories from checkboxes
        if (selectedCategories.isEmpty()) {
            displayError("Please select at least one category."); // Show error if no category is selected
            return;
        }
        ...
        if (writeNewUserToCSV(username, password, selectedCategories)) {
            ...
            SessionManager.setCurrentUser(newUser); // Set the new user in SessionManager
    ...
    private boolean writeNewUserToCSV(String username, String password, List<String> selectedCategories) {
        try (FileWriter writer = new FileWriter(CSV_FILE_PATH, true)) {
            String categories = String.join(";", selectedCategories); // Join selected categories into a string
    ...
    // Method to check if the username is already taken by reading the CSV file
